...
Page({
    ...
    onReady: function () {
        ...
        wx.cloud.callFunction({
            ...
            // 传递给云函数的参数
            data: {
                $url: "signday", // 要调用的路由的路径，传入准确路径或者通配符*
                other: "xxx",
            },
        }).then(res => {
            this.setData({
                signTotal: res.result.data[0].signTotal,
                signday: res.result.data[0].signday,
                integral:res.result.data[0].integral
            })
                    //判断今天是否已经打过卡
            let length = this.data.signday.length
            if (this.data.signday[length - 1] === this.data.todayIndex) {
                this.setData({
                    ornot: true
                })
            }
    ...
    dakainc: function (e) {
        let that = this
        let date = new Date();
        that.data.signday.push(that.data.getDate)
        that.setData({
            ornot: true,
            continuity: that.data.continuity + 1, //连续签到天数
            days: [],
            ...
            signTotal: that.data.signTotal + 1,
            integral:that.data.integral+20
        });
               // 链接云函数获取数据库数据
               wx.cloud.callFunction({
                // 要调用的云函数名称
                ...
                // 传递给云函数的参数
                data: {
                    $url: "sign", // 要调用的路由的路径，传入准确路径或者通配符*
                    signday: this.data.signday,
                    signTotal:this.data.signTotal,
